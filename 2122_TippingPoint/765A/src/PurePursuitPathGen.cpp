...
PurePursuitPathGen::PurePursuitPathGen(double spacing, double a, double b, double tolerance, std::vector<std::pair<double, double>> points, double max_vel, double max_accel, int k) {
    ...
    this->k = k;
    for (std::pair<double, double> x : points) {
        initial_points.push_back({ x.first, x.second, 0, 0, 0 });
    }
}
