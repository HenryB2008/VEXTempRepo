...
// pros::Distance dNorth(1);
// pros::Distance dEast(3);
...
//     const float NORTH_SENSOR_OFFSET_X = -4.0f;
//     const float NORTH_SENSOR_OFFSET_Y = 4.0f;
//     const float NORTH_SENSOR_MOUNT_ANGLE = 0.0f; // 0 radians means facing same as robot's forward

//     const float EAST_SENSOR_OFFSET_X = 6.5f;
//     const float EAST_SENSOR_OFFSET_Y = 0.75f; 
//     const float EAST_SENSOR_MOUNT_ANGLE = -M_PI_2; // 90 degrees in radians

//     const float WEST_SENSOR_OFFSET_X = -6.5f;
//     const float WEST_SENSOR_OFFSET_Y = 1.25f;
//     const float WEST_SENSOR_MOUNT_ANGLE = M_PI_2; // -90 degrees in radians
    

//     lemlib::Pose getSensorWorldPose(const lemlib::Pose& robotPose, 
//             float offset_x, float offset_y, 
//             float sensorMountAngle) {
//     // Combine robot heading with sensor mounting angle.
//     float sensorAngle = robotPose.theta + sensorMountAngle;
//     float sensor_x = robotPose.x + offset_x * std::cos(sensorAngle) - offset_y * std::sin(sensorAngle);
//     float sensor_y = robotPose.y + offset_x * std::cos(sensorAngle) + offset_y * std::sin(sensorAngle);
//     return lemlib::Pose(sensor_x, sensor_y, robotPose.theta);
//     }
// }
...
//     std::normal_distribution<float> rotation_noise(0.0, 0.035);   // 2-degree standard deviation
...
//         */
//     case 'N':
//         sensorPose = getSensorWorldPose(particlePose, NORTH_SENSOR_OFFSET_X, NORTH_SENSOR_OFFSET_Y, NORTH_SENSOR_MOUNT_ANGLE);
//         return FIELD_DIMENSIONS - sensorPose.y;
//    case 'E':
//        sensorPose = getSensorWorldPose(particlePose, EAST_SENSOR_OFFSET_X, EAST_SENSOR_OFFSET_Y, EAST_SENSOR_MOUNT_ANGLE);
//        return FIELD_DIMENSIONS - sensorPose.x;
//    case 'W':
//        sensorPose = getSensorWorldPose(particlePose, WEST_SENSOR_OFFSET_X, WEST_SENSOR_OFFSET_Y, WEST_SENSOR_MOUNT_ANGLE);
//        return sensorPose.x;
//     }
...
// void measurementUpdate(float north_dist, float east_dist, float west_dist) {
...
//                float north_dist, float east_dist, float west_dist) {
//     lemlib::Pose currentPose = chassis.getPose();
//     lemlib::Pose delta = calculateMotionDelta(currentPose);
    
//     motionUpdate(delta);
//     measurementUpdate(north_dist, east_dist, west_dist);
//     resampleParticles();
    
//     lemlib::Pose estimatedPose = getEstimatedPose();
//     xR = estimatedPose.x;
//     yR = estimatedPose.y;
//     thetaR = estimatedPose.theta; 
// }
...
//         updateMCL(*chassisPtr, north, east, west);
