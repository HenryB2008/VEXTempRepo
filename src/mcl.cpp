...
std::vector<double> MCLOdom::get_position() {
  position_mutex.lock();
  std::vector<double> temp = {predicted_x, predicted_y};
  position_mutex.unlock();
  return temp;
}

// Get estimated pose (position and orientation)
lemlib::Pose MCLOdom::get_pose() {
  position_mutex.lock();
  ...
  return lemlib::Pose(x, y, rotation);
}

// Get the robot's current rotation
 double MCLOdom::get_rotation() {
  return (tracker_odom.getPose(true).theta);
}

// Set the robot's rotation
void MCLOdom::set_rotation(double rotation) {
  tracker_odom.setPose(tracker_odom.getPose().x, tracker_odom.getPose().y, rotation);
}
...
void MCLOdom::print_telemetry()
{
  ...
  lemlib::Pose mcl_pose = get_pose();
  xR = mcl_pose.x;
  yR = mcl_pose.y;
  thetaR = mcl_pose.theta; 
  std::cout << "[TELEMETRY.ODOM.MCL]:" << mcl_pose.x << "," << mcl_pose.y << "," << mcl_pose.theta << std::endl;
...
void MCLOdom::start_task(double initial_x, double initial_y,
  ...
  tracker_odom.setPose(tracker_odom.getPose().x, tracker_odom.getPose().y, initial_theta);
